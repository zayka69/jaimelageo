...
d;e[c]=Math.round(d*a[c]/a[b]);return e},Q=function(){k(this).prev("img").trigger("click")},y=function(a,d){var c=a.Elem[a.Current];if(c&&a.Opts.labels)c.Label.el[d?"show":"hide"]()},R=function(a){var d=p[a.Opts.vh],c=a.Elem.length,e=-1,b=0,f,h,j,i=F[d.xy]-a.Elem[0].Wrap.parent().offset()[d.tl];if(i>=0)for(;e<0&&b<c;b++){f=a.Elem[b];h=f.Pad[d.lead]+f.Pad[d.trail];j=f.Major+h;if(i<j){if(b!==a.Current){y(a);a.Current=b}e=f.Offset+i*(f.Initial+h)/j}else i-=j}return e},S=function(a){for(var d=a.childNodes.length,
c;d;){c=a.childNodes[--d];if(c.childNodes&&c.childNodes.length)S(c);else c.nodeType===3&&a.removeChild(c)}},T=function(a){a.Menu.css({visibility:"visible"}).show()},D=function(a){var d=a.Opts.idle;if(d){u(a,0);a[s[0]]=w.setTimeout(function(){a.Menu.trigger("dockidle")},d)}},$=function(a,d,c){var e=a.Opts,b=e.labels,f=d.Label,h;f.el=k('<div class="jqDockLabel jqDockLabel'+d.Link+'" style="position:absolute;margin:0;"></div>').hide().insertAfter(d.Img);if(b){h=b.charAt(0)==="b";b=b.charAt(1)==="r";
f.el.css({top:h?"auto":0,left:b?"auto":0,bottom:h?0:"auto",right:b?0:"auto"}).click(Q)}a=e.setLabel.call(a.Menu[0],d.Title,c,f.el[0]);a!==false&&k('<div class="jqDockLabelText">'+a.toString()+"</div>").appendTo(f.el)},I=function(a,d){var c=l[a],e=c.Opts,b=p[e.vh].wh,f=c.Elem.length,h,j,i;for(d=d||d===0?d:R(c);f--;){h=c.Elem[f];i=h.Initial;if(d>=0){j=Math.abs(d-h.Centre);if(j<e.distance)i=h[b]-Math.round((h[b]-h.Initial)*Math.pow(j,e.coefficient)/e.attenuation);if(f===c.Current){j=[F[p[e.vh].xy],c.Current,
i].join(",");if(j===c.ToFro[0]&&i!==c.ToFro[2])i=c.ToFro[2];else c.ToFro=[c.ToFro[1],j,i]}}h.Final=i}},aa=function(a){return a},U=function(a,d,c,e){a=l[a];var b=a.Elem[d],f=a.Opts,h=a.Yard,j=p[f.vh],i=p[j.inv],n=b.src!==b.altsrc,o,g,m,t;if(e||b.Major!==c){o=k.boxModel||f.vh==="v"?0:a.Border[j.lead]+a.Border[j.trail];if(n&&!e&&b.Major===b.Initial)b.Img[0].src=b.altsrc;a.Spread+=c-b.Major;g=P(b,c,f.vh);m=f.size-g[i.wh];if({top:1,left:1}[f.align])g["margin"+v[i.trail]]=m;else if({middle:1,center:1}[f.align]){t=
Math.round(m*(100-f.bias)/100);g["margin"+v[i.lead]]=m-t;g["margin"+v[i.trail]]=t}else g["margin"+v[i.lead]]=m;if(c!==b.Major||e&&!d){f.flow&&h.parent()[j.wh](a.Spread+a.Border[j.lead]+a.Border[j.trail]);h[j.wh](a.Spread+o)}b.Wrap.css(g);f.flow||h.css(j.tl,Math.floor(Math.max(0,(a[j.wh]-a.Spread)/2)));if(a.OnDock){d=!a.Stamp;f=a.Opts.labels;h=p[a.Opts.vh];j=a.Elem[a.Current];var q,r;if(j&&f){o=j.Label;m=o.el;if(o.mc){o.mc=0;for(q in p){o[q]=m[p[q].wh]();for(r in{lead:1,trail:1})o[q]+=A(m.css("padding"+
...
f,b)}if(d.Spread>d[e.wh]){d.Yard.parent()[e.wh](d.Spread+d.Border[e.lead]+d.Border[e.trail]);d[e.wh]=d.Spread}}},K=function(a,d){var c=l[a],e=c.Elem,b=e.length;u(c,2);if(c.OnDock&&!c.Stamp){for(I(a,d);b&&e[b-1].Major===e[b-1].Final;)--b;if(b){J(a);c[s[2]]=w.setTimeout(function(){K(a,d)},c.Opts.step)}else y(c,1)}},V=function(a,d){var c=l[a],e=c.Elem,b=e.length;if(!c.OnDock){for(;b&&e[b-1].Major<=e[b-1].Initial;)--b;R(c);if(b){J(a);c[s[4]]=w.setTimeout(function(){V(a,d)},c.Opts.step)}else{c.Stamp=0;
for(b=e.length;b--;)e[b].Major=e[b].Final=e[b].Initial;c.Current=-1;d||D(c)}}},W=function(a,d){var c=l[a],e=c.Elem,b=e.length;if(c.OnDock){for(I(a,d);b&&e[b-1].Major===e[b-1].Final;)--b;if(!b||!c.Stamp){c.Stamp=0;K(a,d)}else{J(a);c[s[3]]=w.setTimeout(function(){W(a,d)},c.Opts.step)}}},C=function(a,d,c,e){var b=l[d],f=b.Elem,h=f.length;if(a===0){b.OnDock=1;b.Current>=0&&b.Current!==c&&y(b);b.Current=c;b.Stamp=e&&e>1?0:H(b);W(d,e?f[c].Centre:null)}if(a===1){if(c!==b.Current){y(b);b.Current=c}K(d)}if(a===
